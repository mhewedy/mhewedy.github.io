...
- Co-Designed and implemented https://github.com/mhewedy/spring-data-jpa-mongodb-expressions[`Spring Data JPA MongoDB Expressions` icon:external-link[],role=external,window=_blank] a library that converts MongoDB queries into SQL.
...
* Behavior / Test Driven Development - Unit, Integration, E2E Testing.
...
* A good background in Web Services (SOAP-based and REST) and `XML` (`SAX`, `DOM`, `XSLT`, `XPath`).
...
* Basic knowledge Linux programming (`syscalls`, LKM, LSM, filesystems, namespaces and others).
